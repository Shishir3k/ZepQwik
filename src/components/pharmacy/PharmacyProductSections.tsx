...
export default function PharmacyProductSections() {
  ...
  const fetchPharmacyProducts = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `); 
        // Removed .eq('is_active', true) at the root to allow manual DB entries
      ...
      if (data) {
        ...
        const formattedFeed = targetCategories.map((category: any) => {
          const validSubcategories = (category.subcategories || []).filter(
            // Use !== false so NULL values from manual entries are still displayed
            (sub: any) => sub.is_active !== false && sub.products && sub.products.length > 0
          );
      ...
      console.error('Error fetching pharmacy product sections:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoading) {
    return <ActivityIndicator size="large" color="#059669" className="mt-10 mb-20" />;
  }
